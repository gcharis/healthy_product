...
router.post('/login', (req, res) => {
	...
	Admin.findOne({ username: adminToVerify.username }, (err, admin) => {
		...
		if (!admin)
			return res
				.status(500)
				.send({ message: `Τα στοιχεία που δώσατε είναι λανθασμένα. Παρακαλώ προσπαθήστε ξανά.` });
...
router.get('/logs', (req, res) => {
	readLogs().then((logs) => res.send(logs)).catch((err) => res.send({ message: 'Κάποιο σφάλμα συνέβη.' }));
});
